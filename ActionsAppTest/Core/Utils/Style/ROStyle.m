...
- (void)apply {
    ...
    [SVProgressHUD setFont:[self fontWithSize:[self.fontSizeSmall floatValue]]];
    [SVProgressHUD setForegroundColor:self.foregroundColor];
    [SVProgressHUD setBackgroundColor:self.backgroundColor];
    
    // UITabBar
    
    UITabBar *tabBarAppearance = [UITabBar appearance];
    tabBarAppearance.tintColor = self.applicationBarTextColor;
    tabBarAppearance.barTintColor = [self.applicationBarBackgroundColor colorWithAlphaComponent:0.8f];
    if ([UITabBar instancesRespondToSelector:@selector(unselectedItemTintColor)]) {
        
        tabBarAppearance.unselectedItemTintColor = self.notSelectedColor;
    
    } else {
    
        [[UITabBarItem appearance] setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys: self.applicationBarTextColor, NSForegroundColorAttributeName, nil] forState:UIControlStateSelected];
        
        // set color of unselected text
        [[UITabBarItem appearance] setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:self.notSelectedColor, NSForegroundColorAttributeName, nil]
                                                 forState:UIControlStateNormal];
    }
    
    // UIActivityIndicatorView
    
    UIActivityIndicatorView *indicatorAppearance = [UIActivityIndicatorView appearance];
    indicatorAppearance.color = self.foregroundColor;
}
...
- (void)setNoPhotoImage:(id)noPhotoImage {
    ...
    if ([noPhotoImage isKindOfClass:[UIImage class]]) {
        ...
        UIImage *image = [UIImage imageNamed:noPhotoImage];
        if (!image) {
...
- (UIColor *)notSelectedColor {
    ...
    return [self.applicationBarTextColor colorWithAlphaComponent:0.6f];
}
