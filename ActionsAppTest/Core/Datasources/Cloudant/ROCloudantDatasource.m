...
- (NSMutableDictionary *)queryWithOptionsFilter:(ROOptionsFilter *)optionsFilter {
    ...
    if (self.cloudantManager.filterIndexesCreated) {
        
        if ([optionsFilter.designFilters count] != 0) {
            
            for (NSObject<ROFilter> *filter in optionsFilter.designFilters) {
                
                if ([filter fieldValue] && [filter fieldName]) {
...
- (void)load:(id)identifier onSuccess:(RODatasourceSuccessBlock)successBlock onFailure:(RODatasourceErrorBlock)failureBlock {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        CDTDocumentRevision *rev = [[self.cloudantManager datastore] getDocumentWithId:identifier
                                                                                 error:&error];
            ...
            if (error) {
                ...
                if (successBlock) {
                    ...
                    if (self.objectsClass && [[self.objectsClass alloc] respondsToSelector:@selector(initWithDocumentRevision:)]) {
                        ...
                        obj = [[self.objectsClass alloc] initWithDocumentRevision:rev];
...
- (void)distinctValues:(NSString *)columnName filters:(NSArray *)filters onSuccess:(RODatasourceSuccessBlock)successBlock onFailure:(RODatasourceErrorBlock)failureBlock {
    ...
    successBlock([NSArray new]); // Distinct query is not supported by Cloudant Sync
}
