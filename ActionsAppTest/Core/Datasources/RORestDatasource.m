...
- (void)load:(id)identifier onSuccess:(RODatasourceSuccessBlock)successBlock onFailure:(RODatasourceErrorBlock)failureBlock {
    ...
    NSString *path = [self.resourceId stringByAppendingPathComponent:identifier];
    [self.restClient get:path params:nil responseClass:self.objectsClass successBlock:^(id response) {
...
- (void)distinctValues:(NSString *)columnName filters:(NSArray *)filters onSuccess:(RODatasourceSuccessBlock)successBlock onFailure:(RODatasourceErrorBlock)failureBlock {
    ...
    of.designFilters = [NSMutableArray arrayWithArray:filters];
...
- (NSMutableDictionary *)requestParamsWithOptionsFilter:(ROOptionsFilter *)optionsFilter {
    ...
    if (optionsFilter.designFilters && optionsFilter.designFilters.count != 0) {
        
        NSMutableArray *designFilters = [NSMutableArray new];
        for (NSObject<ROFilter> *filter in optionsFilter.designFilters) {
            
            NSString *qs = [filter getQueryString];
            if (qs) {
                
                [designFilters addObject:[NSString stringWithFormat:@"{%@}", qs]];
            }
        }
        if (designFilters.count != 0) {
            
            [exps addObject:[NSString stringWithFormat:@"\"$and\":[%@]", [designFilters componentsJoinedByString:@","]]];
        }
    }
