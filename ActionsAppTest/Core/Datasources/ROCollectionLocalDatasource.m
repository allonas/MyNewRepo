...
- (void)load:(id)identifier onSuccess:(RODatasourceSuccessBlock)successBlock onFailure:(RODatasourceErrorBlock)failureBlock {
    ...
    [self loadOnSuccess:^(id response) {
        
        NSArray *objects = (NSArray *)response;
        if (successBlock) {
        
            for (id<ROModelDelegate> item in objects) {
                
                NSString *itemID = [item identifier];
                if ([itemID isEqualToString:identifier]) {
    ...
    if (successBlock) {
        ...
        if (results && [results count] != 0) {
            if (optionsFilter) {
                ...
                // design filters
                results = [self objects:results filters:optionsFilter.designFilters];
